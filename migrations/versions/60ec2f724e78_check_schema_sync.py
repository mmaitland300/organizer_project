...
def upgrade() -> None:
    ...
    with op.batch_alter_table('files', schema=None) as batch_op:
        batch_op.alter_column('file_path',
                   ...
                   nullable=False) # Change nullability
        ...
        indexes_to_drop = [
            'ix_files_brightness', 'ix_files_loudness_rms', 'ix_files_mfcc1_mean',
            'ix_files_mfcc2_mean', 'ix_files_mfcc3_mean',
            'ix_files_spectral_contrast_mean', 'ix_files_zcr_mean'
        ]
        ...
        try:
            batch_op.create_index(batch_op.f('ix_files_file_path'), ['file_path'], unique=False)
        except Exception as e: print(f"  ! Warning creating index ix_files_file_path: {e}")
        try:
            batch_op.create_index(batch_op.f('ix_files_size'), ['size'], unique=False)
        except Exception as e: print(f"  ! Warning creating index ix_files_size: {e}")
        ...
        except Exception as e: print(f"  ! Warning creating unique constraint uq_files_file_path: {e}")
...
def downgrade() -> None:
    ...
    with op.batch_alter_table('files', schema=None) as batch_op:
        ...
        except Exception as e: print(f"  ! Warning dropping unique constraint uq_files_file_path: {e}")

        # Drop indexes created in upgrade
        try:
            batch_op.drop_index(batch_op.f('ix_files_size'))
        except Exception as e: print(f"  ! Warning dropping index ix_files_size: {e}")
        try:
            batch_op.drop_index(batch_op.f('ix_files_file_path'))
        except Exception as e: print(f"  ! Warning dropping index ix_files_file_path: {e}")
        ...
        for index_name, columns in indexes_to_recreate:
            ...
            except Exception as e: print(f"  ! Warning recreating index {index_name}: {e}")
        ...
        for col_name in feature_columns_to_revert:
             try:
                 batch_op.alter_column(col_name,
                       ...
                       existing_nullable=True)
             except Exception as e: print(f"  ! Warning reverting column {col_name}: {e}")
        ...
        try:
            batch_op.alter_column('used',
                   ...
                   existing_nullable=True,
                   existing_server_default=sa.text("'0'"))
        except Exception as e: print(f"  ! Warning reverting column used: {e}")

        # Revert 'file_path' nullability
        try:
            batch_op.alter_column('file_path',
                   existing_type=sa.TEXT(),
                   nullable=True) # Revert nullability
        except Exception as e: print(f"  ! Warning reverting column file_path: {e}")
