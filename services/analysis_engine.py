...
class AnalysisEngine:
    ...
    def analyze_audio_features(
        ...
        try:
            ...
            logger.error(f"Failed to instantiate SpectrogramService (missing dependencies?): {e}")
            return {}

        # --- Get Spectrogram and Audio Data ---
        logger.debug(f"Getting spectrogram data for first <= {max_duration}s of {os.path.basename(file_path)}")
        try:
            spec_data = the_service.get_spectrogram_data(file_path, load_duration=max_duration)
        except Exception as spec_e:
             logger.error(f"Unexpected error calling get_spectrogram_data for {os.path.basename(file_path)}: {spec_e}", exc_info=True)
             return {} # Cannot proceed without data

        # --- Validate Service Result ---
        if spec_data.get('error'):
            logger.error(f"SpectrogramService failed for {os.path.basename(file_path)}: {spec_data['error']}")
            return {}
        if not isinstance(spec_data.get('sr'), int) or spec_data.get('y') is None:
            logger.error(f"SpectrogramService returned invalid data (missing sr or y) for {os.path.basename(file_path)}")
            return {}
        ...
        if 'bpm' in features:
            ...
            try:
                ...
                features['bpm'] = float(tempo_result[0]) if tempo_result.size > 0 else None
            except Exception as e: logger.warning(f"BPM failed for {basename}: {e}", exc_info=False); features['bpm'] = None
        ...
        if 'brightness' in features:
            ...
            try:
                if S_magnitude is not None and S_magnitude.size > 0:
                    ...
                    features["brightness"] = float(np.mean(centroid[np.isfinite(centroid)])) # Filter non-finite values
                else: logger.warning(f"Magnitude spectrogram missing or empty for brightness: {basename}")
            except Exception as e: logger.warning(f"Brightness failed for {basename}: {e}", exc_info=False)
        ...
        if 'loudness_rms' in features:
            ...
            try:
                ...
                features["loudness_rms"] = float(np.mean(finite_rms)) if finite_rms.size > 0 else None
            except Exception as e: logger.warning(f"Loudness_rms failed for {basename}: {e}", exc_info=False)
        ...
        if 'zcr_mean' in features:
            ...
            try:
                ...
                features["zcr_mean"] = float(np.mean(finite_zcr)) if finite_zcr.size > 0 else None
            except Exception as e: logger.warning(f"ZCR failed for {basename}: {e}", exc_info=False)
        ...
        if 'spectral_contrast_mean' in features:
            ...
            try:
                if S_magnitude is not None and S_magnitude.size > 0:
                    ...
                    features["spectral_contrast_mean"] = float(np.mean(finite_contrast)) if finite_contrast.size > 0 else None
                else: logger.warning(f"Magnitude spectrogram missing or empty for spectral contrast: {basename}")
            except Exception as e: logger.warning(f"Spectral Contrast failed for {basename}: {e}", exc_info=False); features['spectral_contrast_mean'] = None
        ...
        if f'mfcc1_mean' in features: # Check if MFCCs are expected
            ...
            try:
                if S_mel is not None and S_mel.size > 0:
                    ...
                    mfccs = librosa.feature.mfcc(S=S_mel_db, sr=sr, n_mfcc=n_mfcc_to_use)
                    ...
                    for i in range(min(n_mfcc_to_use, mfccs.shape[0])): # Iterate over actual number of coefficients calculated
                        key = f"mfcc{i+1}_mean"
                        if key in features:
                            finite_coeffs = mfccs[i, np.isfinite(mfccs[i,:])]
                            features[key] = float(np.mean(finite_coeffs)) if finite_coeffs.size > 0 else None
                else: logger.warning(f"Mel spectrogram missing or empty for MFCC calculation: {basename}")
            except Exception as e: logger.warning(f"MFCCs failed for {basename}: {e}", exc_info=False)
            ...
            if SOUNDFILE_AVAILABLE and sf is not None:
                try:
                    ...
                    if bit_depth_val is None and 'float' in info.subtype.lower():
                        bit_depth_val = 32 # Assume 32-bit float if subtype indicates float
                    features['bit_depth'] = bit_depth_val
                except Exception as e: logger.warning(f"Bit depth analysis failed for {basename}: {e}", exc_info=False); features['bit_depth'] = None
            else:
            ...
            if PYLOUDNORM_AVAILABLE and pyln is not None:
                try:
                    ...
                    if np.any(y):
                         ...
                         logger.debug(f"Audio data empty or silent, skipping LUFS for {basename}")
                         features['loudness_lufs'] = None
                except Exception as e: logger.warning(f"LUFS calculation failed for {basename}: {e}", exc_info=False); features['loudness_lufs'] = None
            else:
        ...
        if 'pitch_hz' in features:
            ...
            except Exception as e: logger.warning(f"Pitch (pyin) failed for {basename}: {e}", exc_info=False); features['pitch_hz'] = None
        ...
        if 'attack_time' in features:
            ...
            try:
                ...
                onsets_frames = librosa.onset.onset_detect(onset_envelope=onset_env,
                                                           sr=sr,
                                                           units='frames',
                                                           wait=1, pre_avg=1, post_avg=1, pre_max=1, post_max=1) # Use librosa defaults
                # Convert first detected onset frame to time (seconds)
                first_onset_time_sec = librosa.frames_to_time(onsets_frames[0], sr=sr) if len(onsets_frames) > 0 else None
                features['attack_time'] = float(first_onset_time_sec) if first_onset_time_sec is not None else None
            except Exception as e: logger.warning(f"Attack time failed for {basename}: {e}", exc_info=False); features['attack_time'] = None
