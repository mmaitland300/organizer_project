...
if not NUMPY_AVAILABLE: logger.error("numpy not installed. SpectrogramService disabled.")
if not LIBROSA_AVAILABLE: logger.error("librosa not installed. SpectrogramService disabled.")
...
class SpectrogramService:
    ...
    def __init__(self):
        ...
        if not NUMPY_AVAILABLE or not LIBROSA_AVAILABLE:
            raise ImportError("SpectrogramService requires numpy and librosa to be installed.")
        logger.info(f"SpectrogramService initialized with cache size: {SPECTROGRAM_CACHE_SIZE}")
    ...
    def _compute_spectrogram_data(
        ...
        """
        logger.debug(f"Computing spectrogram for: {os.path.basename(file_path)} with params: n_fft={n_fft}, hop={hop_length}, win={win_length}, window='{window}'")
        results: Dict[str, Any] = {
            ...
            'win_length': win_length if win_length is not None else n_fft, # Store actual win_length used
            'window': window,
            'error': None,
        }
        try:
            ...
            results['mel'] = S_mel
            logger.debug(f"Successfully computed spectrogram for {os.path.basename(file_path)}")

        except FileNotFoundError as fnf_err:
             logger.error(f"File not found error during spectrogram calculation: {fnf_err}")
             results['error'] = str(fnf_err)
        except Exception as e:
            logger.error(f"Error computing spectrogram for {file_path}: {e}", exc_info=True)
    ...
    def get_spectrogram_data(
        ...
        **stft_params_override: Any
        ...
        """
        if not NUMPY_AVAILABLE or not LIBROSA_AVAILABLE:
            logger.warning("SpectrogramService dependencies not met. Returning empty dict.")
        ...
        win_length = STFT_WIN_LENGTH if win_length_override == 'DEFAULT_SENTINEL' else win_length_override
        ...
        if results.get('error'):
            logger.warning(f"Spectrogram data retrieval failed for {os.path.basename(file_path)}: {results['error']}")
    ...
    def get_cache_info(self):
        ...
        if not hasattr(self, '_compute_spectrogram_data') or not hasattr(self._compute_spectrogram_data, 'cache_info'):
            return "Cache not available."
        return self._compute_spectrogram_data.cache_info()

    def clear_cache(self):
        """Clears the LRU cache."""
        if hasattr(self, '_compute_spectrogram_data') and hasattr(self._compute_spectrogram_data, 'cache_clear'):
