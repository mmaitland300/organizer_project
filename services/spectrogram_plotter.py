...
class SpectrogramPlotter:
    ...
    @staticmethod
    def _apply_theme_to_axes(ax: Axes, figure: Figure, theme: str, colorbar: Optional[Any] = None):
        ...
        try:
            ...
            if colorbar:
                colorbar.ax.yaxis.set_tick_params(color=text_color, labelcolor=text_color)
    ...
    def plot(file_path: str, ax: Axes, figure: Figure, theme: str = 'light') -> bool:
        ...
        # --- Dependency Checks ---
        if not all([MATPLOTLIB_AVAILABLE, NUMPY_AVAILABLE, LIBROSA_AVAILABLE, SERVICE_AVAILABLE]):
            msg = "Missing dependencies for plotting spectrogram."
            logger.error(msg)
            ax.cla()
            ax.text(0.5, 0.5, msg, ha='center', va='center', wrap=True, color=text_color)
            SpectrogramPlotter._apply_theme_to_axes(ax, figure, theme) # Style error message
            return False
        ...
        try:
            ...
            ax.text(0.5, 0.5, f"Error getting spectrogram data:\n{e}", ha='center', va='center', wrap=True, color=text_color)
            ax.set_title(f"Spectrogram Error", color=text_color) # Set title color here too
            SpectrogramPlotter._apply_theme_to_axes(ax, figure, theme)
            return False

        # --- Handle errors from service ---
        basename = os.path.basename(file_path)
        if spec_data.get('error') or 'magnitude' not in spec_data or spec_data['magnitude'] is None or 'sr' not in spec_data or spec_data['sr'] is None:
            error_msg = spec_data.get('error', 'Missing spectrogram data (magnitude/sr).')
            logger.warning(f"Cannot plot spectrogram for {basename}: {error_msg}")
            ax.text(0.5, 0.5, f"Could not generate spectrogram:\n{error_msg}", ha='center', va='center', wrap=True, color=text_color)
        ...
        try:
            ...
            logger.error(f"Error plotting spectrogram for {file_path}: {plot_err}", exc_info=True)
            # Display error on the plot itself
            ax.cla() # Clear potentially partial plot
            ax.text(0.5, 0.5, f"Error during plotting:\n{plot_err}", ha='center', va='center', wrap=True, color=text_color)
            ax.set_title(f"Spectrogram: {basename} (Plotting Error)", color=text_color)
            SpectrogramPlotter._apply_theme_to_axes(ax, figure, theme, cbar) # Apply theme to error text too
            return False