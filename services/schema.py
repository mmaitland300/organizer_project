...
try:
    ...
    logger.error("Could not import feature keys from settings. Using fallback list.", exc_info=True)
...
files_table = Table(
    ...
    *(Column(feature, Float, nullable=True) for feature in ALL_FEATURE_KEYS
      if feature not in ['bit_depth']), # Exclude bit_depth as it's Integer
