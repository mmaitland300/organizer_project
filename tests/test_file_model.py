...
def test_data_display(file_model: FileTableModel):
    ...
    col_index = -1
    try: col_index = file_model.COLUMN_HEADERS.index("File Name")
    except ValueError: pytest.fail("Column 'File Name' not found")
    assert col_index != -1
...
def test_setData_edit(file_model: FileTableModel):
    ...
    key_col_index = -1
    try: key_col_index = file_model.COLUMN_HEADERS.index("Key")
    except ValueError: pytest.fail("Column 'Key' not found")
...
@pytest.mark.parametrize("query_string, expected_structure", [
    # Basic terms (default AND, default fields)
    ("kick", [{'term': 'kick', 'fields': ['name', 'tag'], 'negated': False, 'op': 'AND'}]),
    ("kick snare", [
        {'term': 'kick', 'fields': ['name', 'tag'], 'negated': False, 'op': 'AND'},
        {'term': 'snare', 'fields': ['name', 'tag'], 'negated': False, 'op': 'AND'}
    ]),
    # Quoted terms
    ('"hi hat"', [{'term': 'hi hat', 'fields': ['name', 'tag'], 'negated': False, 'op': 'AND'}]),
    ('kick "808 snare"', [
        {'term': 'kick', 'fields': ['name', 'tag'], 'negated': False, 'op': 'AND'},
        {'term': '808 snare', 'fields': ['name', 'tag'], 'negated': False, 'op': 'AND'}
    ]),
    # Boolean Operators
    ("kick AND snare", [
        {'term': 'kick', 'fields': ['name', 'tag'], 'negated': False, 'op': 'AND'},
    ...
    ("kick OR snare", [
        {'term': 'kick', 'fields': ['name', 'tag'], 'negated': False, 'op': 'AND'}, # First term defaults to AND link conceptually
        {'term': 'snare', 'fields': ['name', 'tag'], 'negated': False, 'op': 'OR'}
    ]),
    ("loop AND NOT acoustic", [
        {'term': 'loop', 'fields': ['name', 'tag'], 'negated': False, 'op': 'AND'},
        {'term': 'acoustic', 'fields': ['name', 'tag'], 'negated': True, 'op': 'AND'}
    ]),
    ("NOT closed hat", [
        {'term': 'closed', 'fields': ['name', 'tag'], 'negated': True, 'op': 'AND'},
        {'term': 'hat', 'fields': ['name', 'tag'], 'negated': False, 'op': 'AND'} # NOT applies only to 'closed'
    ]),
    # Field Specifiers
    ("tag:kick", [{'term': 'kick', 'fields': ['tag'], 'negated': False, 'op': 'AND'}]),
    ("name:loop", [{'term': 'loop', 'fields': ['name'], 'negated': False, 'op': 'AND'}]),
    ("key:Cm", [{'term': 'Cm', 'fields': ['key'], 'negated': False, 'op': 'AND'}]),
    ('path:/samples/drums', [{'term': '/samples/drums', 'fields': ['path'], 'negated': False, 'op': 'AND'}]),
    ('tag:"hi hat"', [{'term': 'hi hat', 'fields': ['tag'], 'negated': False, 'op': 'AND'}]),
    # Combinations
    ...
    # Invalid field
    ("invalidfield:value", [{'term': 'value', 'fields': ['name', 'tag'], 'negated': False, 'op': 'AND'}]), # Falls back to default search
])
...
def run_advanced_filter(proxy_model, db_manager, query, file_info_list):
    ...
    source_model = FileTableModel(populated_file_info, db_manager=db_manager, size_unit="KB")
...
def test_adv_filter_quoted_term(adv_proxy_model, db_manager): # <<< Added db_manager if needed by helper
    """Test filtering with a quoted term (searches for exact substring)."""
    query = '"hi hat"'
    files = [
        {'path': '/s/closed_hi hat_01.wav', 'tags': {}}, # Match name (exact phrase)
        {'path': '/s/open_hat_02.wav', 'tags': {'instr': ['HI HAT']}}, # Match tag (exact phrase)
        {'path': '/s/loop_with_hi_hats.wav', 'tags': {}}, # Does NOT contain exact phrase "hi hat"
...
def test_adv_filter_field_specifier(adv_proxy_model, db_manager): # <<< Added db_manager
    """Test filtering with field specifiers (name:, tag:, key:)."""
    query = "name:loop tag:ambient key:Am"
    files = [
        {'path': '/s/ambient_loop_01.wav', 'tags': {'mood': ['AMBIENT'], 'type': ['LOOP']}, 'key': 'Am'},
        {'path': '/s/dark_loop_pad.wav', 'tags': {'mood': ['AMBIENT', 'DARK']}, 'key': 'Cm'},
...
def test_adv_filter_tag_field_logic(adv_proxy_model, db_manager): # <<< Added db_manager
    """Test tag: field specifier searches all tag values."""
    query = "tag:loop"
    files = [
        {'path': '/s/beat.wav', 'tags': {'type': ['LOOP', 'DRUMS']}},
        {'path': '/s/melody.wav', 'tags': {'instrument': ['SYNTH'], 'form': ['MELODY LOOP']}},
...
def test_adv_filter_combined_with_other_filters(adv_proxy_model, db_manager): # <<< Added db_manager
