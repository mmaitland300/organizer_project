...
if MOCK_LIBS or 'numpy' not in sys.modules:
    ...
    sys.modules['numpy'].abs = mock.MagicMock(side_effect=lambda x: np.core.umath.absolute(x) if isinstance(x, np.ndarray) else abs(x)) # More realistic abs
    sys.modules['numpy'].array = np.array # Use real numpy array if available but mocked
...
try:
    from config.settings import (
        STFT_N_FFT, STFT_HOP_LENGTH, STFT_WIN_LENGTH, STFT_WINDOW,
...
class TestSpectrogramService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            if hasattr(SpectrogramService, '_compute_spectrogram_data') and hasattr(SpectrogramService._compute_spectrogram_data, 'cache_clear'):
            ...
            self.skipTest("Skipping test: numpy or librosa not installed/mocked properly.")
    ...
    def test_calculation_success(self, mock_melspectrogram, mock_stft, mock_load, mock_exists):
        ...
        mock_complex_stft = np.array([[1+1j, 2+2j], [3+3j, 4+4j]], dtype=complex)
        ...
        expected_win_length = STFT_WIN_LENGTH if STFT_WIN_LENGTH is not None else STFT_N_FFT
    ...
    @mock.patch('services.spectrogram_service.SpectrogramService._compute_spectrogram_data')
...
if __name__ == '__main__':
    ...
    if 'numpy' not in sys.modules:
        sys.modules['numpy'] = mock.MagicMock(spec=['abs', 'array', 'float32'])
        sys.modules['numpy'].abs = mock.MagicMock(side_effect=lambda x: np.core.umath.absolute(x) if isinstance(x, np.ndarray) else abs(x))
