...
from services.advanced_analysis_worker import AdvancedAnalysisWorker, _analyze_file_process_worker # Import the real worker function for type checking if needed
...
def mock_analyze_process_worker_side_effect(file_info: Dict, cancel_event: MagicMock) -> Optional[Dict]:
    """
    ...
    if path == "/dummy/audio1.wav":
        updated_info.update({"brightness": 1000.0, "loudness_rms": 0.5, "pitch_hz": 440.0})
        return updated_info
    elif path == "/dummy/audio2.flac":
        updated_info.update({"brightness": 2000.0, "loudness_rms": 0.7, "pitch_hz": 880.0})
        return updated_info
    elif path == "/dummy/audio_error.mp3": return None
    elif path == "/dummy/audio_no_features.aiff": return None
    elif path == "/dummy/non_audio.txt": return None
    else: return None
...
def test_parallel_analysis_and_db_save(
    ...
    files = [
        {"path": "/dummy/audio1.wav", "tags": {"filetype": [".wav"]}, "bpm": 120, "size": 100, "mod_time": 1.0},
        {"path": "/dummy/non_audio.txt", "tags": {"filetype": [".txt"]}, "bpm": None, "size": 10, "mod_time": 1.0},
        {"path": "/dummy/audio2.flac", "tags": {"filetype": [".flac"]}, "bpm": None, "size": 200, "mod_time": 1.0},
        {"path": "/dummy/audio_error.mp3", "tags": {"filetype": [".mp3"]}, "bpm": None, "size": 50, "mod_time": 1.0},
        {"path": "/dummy/audio_no_features.aiff", "tags": {"filetype": [".aiff"]}, "bpm": None, "size": 150, "mod_time": 1.0},
    ]
    ...
    if exit_code != 0:
        worker.cancel()
        pytest.fail(f"Worker did not finish within {TIMEOUT_MS}ms timeout (exit code: {exit_code})")

    # --- Assert Mock Submit Calls (Using mock_aliased_submit) ---
    assert mock_aliased_submit.call_count == total_files, \
    ...
    assert any(args[0] == total_files and args[1] == total_files for args in spy_progress), \
    ...
    for record in saved_list:
        ...
        if record["path"] == "/dummy/audio1.wav": assert record.get("bpm") == 120
...
def test_cancellation(mock_submit, mock_save_records, db_manager: DatabaseManager):
    ...
    files = [
        {"path": "/dummy/cancel1.wav", "tags": {"filetype": [".wav"]}, "size": 1, "mod_time": 1},
        {"path": "/dummy/cancel2.wav", "tags": {"filetype": [".wav"]}, "size": 1, "mod_time": 1},
        {"path": "/dummy/cancel3.wav", "tags": {"filetype": [".wav"]}, "size": 1, "mod_time": 1},
    ]
